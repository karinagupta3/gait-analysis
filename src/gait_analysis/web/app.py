...
.note{{color:#777;font-size:13px}}</style></head><body>
<h1>Process a video</h1>
<p class="note">Quick mode = one phone (MediaPipe 3D -> OpenSim IK). Requires OpenSim + a marked
model via the <code>GAIT_OSIM_MODEL</code> env var. Accurate (2-phone) capture: use the CLI for now.</p>
<form action="/process" method="post" enctype="multipart/form-data">
  <label>Subject</label><input name="subject" placeholder="e.g. J. Smith">
  <label>Trial label</label><input name="trial" placeholder="e.g. squats">
...
def create_app(process_fn=None):
    ...
    async def process(video: UploadFile, subject: str = Form(""), trial: str = Form(""),
                      speed: str = Form(""), mode: str = Form("quick")):
        sid = uuid.uuid4().hex[:8]
        sdir = _store_dir() / sid
        sdir.mkdir(parents=True, exist_ok=True)
        ...
        meta = {"id": sid, "subject": subject, "trial": trial,
